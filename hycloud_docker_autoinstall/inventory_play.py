...
#写日志
def log_output(log=None):
    with open(os.path.join(BaseFolder, "run_python.log"), 'a') as f:
...
def run_first(group=None):
    with open(os.path.join(workpath, "inventory_%s") % (group), 'w') as f:
        f.write("[" + group + "]" + "\n")
        # print(group)
        # print(SectionDict_app[group])
        f.write(":".join(SectionDict_app[group][0]) + '\n')
    if group not in inter_no_install:
        print("执行命令：ansible-playbook -i inventory_%s install_%s.yml" % (group, group))
        log_output(log="执行命令：ansible-playbook -i inventory_%s install_%s.yml" % (group, group))
...
# 取出：var组名
        for section_var in SectionDict_var:
            if section + ":vars" == section_var:
                ...
                if "nginx_net" in SectionDict_app:
...
# 先跑mariadb
                for data in SectionDict_app_full:
                    ...
                    if "nginx" in data:
                        continue
                    if ":vars" not in data:
                        if data not in inter_no_install:
                            yaml_file = "install_" + data + ".yml"
                            print("执行命令：ansible-playbook -i inventory_%s %s" % (data, yaml_file))
                            log_output(log="执行命令：ansible-playbook -i inventory_%s %s" % (data, yaml_file))
...
def trs_run():
    ...
    for app in SectionDict_trs:
        ...
        for group, groupvalue in SectionDict_trs[app]:
            # print(group)
            # print(groupvalue)
            with open(os.path.join(workpath, "inventory_trs_" + group), 'w') as f:
                f.write("[%s]\n" % group)
                for i in groupvalue:
                    if ":var" in group:
                        f.write("=".join(i) + "\n")
...
#ids_net使用nginx——net
            ...
            if ":vars" not in group:
                ...
                if group not in inter_no_install:
                    ...
                    log_output(log="执行命令：ansible-playbook -i %s  %s" % ("inventory_trs_" + group, yaml_file))
                    trs_result = subprocess.call("ansible-playbook -i %s  %s" % ("inventory_trs_" + group, yaml_file),
                                                 cwd=workpath, shell=True)
                    if trs_result != 0:
                         exit(2)
                else:
...
def hy_run():
    ...
    for app in SectionDict_hy:
        for group, groupvalue in SectionDict_hy[app]:
            # print(group)
            # print(groupvalue)
            with open(os.path.join(workpath, "inventory_hy_" + group), 'w') as f:
                f.write("[%s]\n" % group)
                for i in groupvalue:
                    f.write(":".join(i) + "\n")
                if "iip" in group:
...
#                        f.write("[elasticsearch_cluster]\n")
            ...
            if group not in inter_no_install:
                ...
                log_output(log="执行命令：ansible-playbook -i %s  %s" % ("inventory_hy_" + group, yaml_file))
                hy_result = subprocess.call("ansible-playbook -i %s  %s" % ("inventory_hy_" + group, yaml_file),
                                            cwd=workpath, shell=True)
                if hy_result != 0:
                    exit(2)
            else:
...
if __name__ == "__main__":
    ...
    log_output(log="-" * 20 + "开始为所有主机安装docker环境" + "-" * 20)
    print("执行命令：ansible-playbook -i inventory-all install_docker.yml")
    log_output(log="执行命令：ansible-playbook -i inventory-all install_docker.yml")
    ...
    # 拿到目录下的所有文件
    #初始化互联网安装脚本
    subprocess.call("> %s" % os.path.join(workpath, "inter.sh"), shell=True)
    subprocess.call("chmod 777 %s" % os.path.join(workpath, "inter.sh"), shell=True)

    print("-" * 20 + "开始安装基础应用" + "-" * 20)
    log_output(log="-" * 20 + "开始安装基础应用" + "-" * 20)
    for TmpBasePath, TmpFolders, TmpFiles in os.walk(os.path.join(BaseFolder, 'tmp')):
        for file in TmpFiles:
            ...
            log_output(log="读取inventory文件：" + os.path.join(BaseFolder, "tmp", file))
            # 调用函数，安装基础
            ParseFile(os.path.join(BaseFolder, "tmp", file))
# TRS
    print("-" * 20 + "开始安装trs应用" + "-" * 20)
    log_output(log="-" * 20 + "开始安装trs应用" + "-" * 20)
    trs_run()
